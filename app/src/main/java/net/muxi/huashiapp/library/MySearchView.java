    ...
    private void startFilter(CharSequence s) {
        if (mAdapter != null && mAdapter instanceof Filterable) {
            if (mAdapter.getCount() > 0) {
                ((Filterable) mAdapter).getFilter().filter(s, MySearchView.this);
            }else {
                ((Filterable) mAdapter).getFilter();
            }
        }
    }
...
//        if (mSearchSrcTextView.getText().length() > 0) {
...
//        }
    ...
    public void setSuggestions(String[] suggestions) {
        if (suggestions != null && suggestions.length > 0) {
            ...
            final MySearchAdapter adapter = new MySearchAdapter(mContext, suggestions, suggestionIcon, ellipsize);
            setAdapter(adapter);
